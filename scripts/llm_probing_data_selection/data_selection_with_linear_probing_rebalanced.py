...
from w2s.data_source_selection_linear_probing import (
    ...
    partition_indices_with_count,
    setup_arms,
    StrategyTracker
)
import argparse

save_folder = '../../results/data_selection_linear_probing_rebalanced_eval/'
def parse_args():
...
if os.path.exists(f'{save_folder}/data_selection_with_linear_probing_results_{dataset_name}_{seed}.json'):
    print("The result exists already!")
    print(f"Loading results from {save_folder}/data_selection_with_linear_probing_results_{dataset_name}_{seed}.json")
    with open(f'{save_folder}/data_selection_with_linear_probing_results_{dataset_name}_{seed}.json', 'r') as f:
        result = json.load(f)
    print(result)
    exit()
else:
    ...
    x_w2s_train_nonhard_augmented = x_w2s_train_nonhard
    detected_easy_indices, detected_overlap_indices = detect_overlap_easy(x_w2s_train_nonhard, x_w2s_train_hard, x_w2s_train_nonhard, detected_nonhard_indices)
    ...
    overlap_count_list = [int(len(detected_overlap_indices) * ratio) for ratio in overlap_sampling_ratio_list]
    easy_count_list = []
    hard_count_list = []
    for x, p in zip(overlap_count_list, overlap_sampling_ratio_list):
        easy_count_list.append(int(x * (1-p) / (2*p)))
        hard_count_list.append(int(x * (1-p) / (2*p)))
    if len(easy_count_list) > len(detected_easy_indices):
        print("Warning: easy_count_list is larger than detected_easy_indices. Perform uniform sampling for easy indices.")
        easy_count_list = [int(len(detected_easy_indices) * 1/K) for _ in range(K)]
    if len(hard_count_list) > len(detected_hard_indices):
        print("Warning: hard_count_list is larger than detected_hard_indices. Perform uniform sampling for hard indices.")
        hard_count_list = [int(len(detected_hard_indices) * 1/K) for _ in range(K)]

    detected_overlap_partition = partition_indices_with_count(detected_overlap_indices, overlap_count_list, seed)
    detected_easy_partition = partition_indices_with_count(detected_easy_indices, easy_count_list, seed)
    detected_hard_partition = partition_indices_with_count(detected_hard_indices, hard_count_list, seed)
    ...
    for i in tqdm(range(K)):
        source = i
        arm = arms[source]
        ucb_arm_choice_log.append(int(source))
        X_train_sample, X_weak_train_sample, y_train_sample = arm.get_samples()
        with torch.no_grad():
            y_train_sample_pseudo = weak_model.predict(X_weak_train_sample)
    ...
    for i in tqdm(range(T)):
        ...
        random_arm_choice_log.append(int(source))
    ...
    result = {
        ...
        'random_arm_choice_log': random_arm_choice_log,
        'ucb_ucb_log': ucb_ucb_log,
        'ucb_arm_choice_log': ucb_arm_choice_log,
        'random_arm_choice_log': random_arm_choice_log,
        'overlap_density_list': overlap_density_list,
        'detected_easy_partition': [partition.astype(int).tolist() for partition in detected_easy_partition]    ,
        'detected_hard_partition': [partition.astype(int).tolist() for partition in detected_hard_partition],
        'detected_overlap_partition': [partition.astype(int).tolist() for partition in detected_overlap_partition],
    ...
    with open(f'{save_folder}/data_selection_with_linear_probing_results_{dataset_name}_{seed}.json', 'w') as f:
        json.dump(result, f)